...
```powershell
dotnet-coverage collect dotnet test ".\AdaskoTheBeAsT.Interop.slnx" --no-build -s ".\coverage.settings.xml" -f xml -o ".\coverage.xml"
```

### Cobertura output

```powershell
dotnet-coverage collect dotnet test ".\AdaskoTheBeAsT.Interop.slnx" --no-build -s ".\coverage.settings.xml" -f cobertura -o ".\coverage.cobertura.xml"
```
